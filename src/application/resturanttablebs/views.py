...
def Search_Booking(request):
    if request.method == "GET":
        ...
        if query:
            ...
            return render(request, 'search-booking.html', {'tb': tb, 'query': query})
        else:
...
def Check_Booking_Status(request):
    if request.method == "GET":
        ...
        if query:
            ...
            messages.success(request, f"Search results for '{query}'")
            return render(request, 'booking-status.html', {'tb': tb, 'query': query})
        else:
            
            return render(request, 'booking-status.html', {})

def VIEW_BOOKING_STATUS(request,bookingnumber):    
    view_booking_status = Tablebooking.objects.filter(bookingnumber=bookingnumber)
    context = {
